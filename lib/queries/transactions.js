...
function buildTransactionWhere(params, tableAlias = '') {
  ...
  if (view === 'needs-review') where.push(`(${prefix}reviewed = 0 AND (${prefix}ai_confidence < 0.5 OR ${prefix}ai_confidence IS NULL OR ${prefix}classification_source = 'pending' OR ${prefix}owner_primary = '待確認' OR ${prefix}category_primary = '待確認' OR ${prefix}necessity = '需確認'))`);
...
function getMeta() {
  ...
  return {
    ...
    counts: {
      ...
      needsReview: db.prepare(`SELECT COUNT(*) AS count FROM transactions WHERE reviewed = 0 AND (ai_confidence < 0.5 OR ai_confidence IS NULL OR classification_source = 'pending' OR owner_primary = '待確認' OR category_primary = '待確認' OR necessity = '需確認')`).get().count,
    },
