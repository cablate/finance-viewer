...
function readinessForGoal(goal, { entityKey = 'personal', asOfDate = localDate() } = {}, db = getDb()) {
  ...
  if (goal === 'net_worth') {
    ...
    const gaps = [cash, debt, investments].filter((item) => item.status !== 'complete').map((item) => ({ gap: `${item.goal}_not_ready`, status: item.status }));
    return { goal, status: gaps.length ? 'partial' : 'complete', as_of_date: asOfDate, gaps, candidate_gaps: expectations.candidates, prerequisites: { cash_position: cash.status, debt_obligations: debt.status, investment_value: investments.status }, report_available: false };
  }
...
function getFinanceInventory({ entityKey = 'personal', asOfDate = localDate() } = {}, db = getDb()) {
  ...
  return {
    ...
    credit_cards: creditCards, liabilities, commitments, investments, transaction_coverage: transactionCoverage,
    readiness: {
