...
function listHumanConfirmations({ status = 'pending' } = {}, db = getDb()) {
  ...
  db.prepare("UPDATE human_confirmation_requests SET status='expired' WHERE status IN ('pending','confirmed') AND expires_at <= ?").run(new Date().toISOString());
  return db.prepare(`SELECT proposal_key,action_kind,resource_type,resource_key,payload_hash,payload_json,expected_version,status,expires_at,confirmed_at,consumed_at,created_at FROM human_confirmation_requests ${status === 'all' ? '' : 'WHERE status=?'} ORDER BY created_at DESC`).all(...(status === 'all' ? [] : [status])).map((row)=>({...row,payload:JSON.parse(row.payload_json),payload_json:undefined}));
}
