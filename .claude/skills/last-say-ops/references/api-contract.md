...
- `POST /api/finance/analysis-context`: read-only named dataset batch. Request `{entity?,as_of?,datasets:[{name,...allowed_filters}]}`. Allowed names are advertised by capabilities: `cash_activity`, `account_balances`, `debt_obligations`, `investment_positions`, `valued_items`, `reconciliation`, and `net_worth_inventory`. Limits are dataset-specific; arbitrary SQL/table/column expressions fail closed. Responses include policy, source, and resource watermarks and exclude source filenames, raw payloads, and content hashes.
